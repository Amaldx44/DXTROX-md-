...
module.exports = {
    async handler(chatUpdate) {
        ...
        const ultahh = new Date('January 5, 2023 23:59:59')
        ...
        global.ulangTahun = `Not enough${ohari} Days ${ojam} O'clock ${onet} Minutes ${detek} Seconds Again For My Birthday🥰`
        global.tahunBaru = `Not enough ${jhari} Days ${jjam} O'clock ${mmmenit} Minute ${ddetik} New Year's Time🥳`
        ...
        let ne = await (await fetch('https://raw.githubusercontent.com/SudoAnirudh/RESULTS/master/media/pic.txt')).text()
        let nek = ne.split('\n')
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                        let cap = `Your limit is up, please buy via ${usedPrefix}buy or ${usedPrefix}buyall Be a premium user so that your limit is unlimited, if interested please type #uptoprem`
                    ...
                    if (plugin.level > _user.level) {
                        let cap = `Level required ${plugin.level} to use this command. Your level ${_user.level}, raise your level by typing${usedPrefix}levelup or click the button below!` 
                        let caption = await this.trans(lang, cap).catch(async _ => await this.trans2(lang, cap))
                    ...
                    try {
                        ...
                        let lim = await this.trans(lang, 'Limits apply').catch(async _ => await this.trans2(lang, 'Limit terpakai'))
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            this.sendBD(id, text, wm, pp, [[`Menu`, `.menu`], [action === 'add' ? 'Welcome 🙏' : 'Goodbye 👋', '@Eva']], {                      
                            ...
                            //key: { fromMe: false, participant: '0@s.whatsapp.net', remoteJid: 'status@broadcast' }, message: { contactMessage: { displayName: `${await this.getName(user)}`, vcard: `BEGIN: VCARD\nVERSION:3.0\nN:;WA;;;\nFN: WA\nTEL ; type=VOICE;waid=${user.split('@')[0]}:${user.split('@')[0]}\nEND:VCARD`}}}, 
                               //...options 
                               { 
                               jpegThumbnail: await (await fetch("https://telegra.ph/file/1245227d550a493c2c516.jpg")).buffer(), fileName: action === 'add' ? `Welcome ${this.getName(user)} 🥰` : `Goodbye ${this.getName(user)} ☺`, mimetype: global.td, fileLength: global.fsdx, pageCount: global.pcdx,
                               mentions: [user],
                               contextInfo: {
                               externalAdReply :{
                                  mediaUrl: linkig,
...
`
        this.sendB(msg.key.remoteJid, caption, wm, null, [[`Turn on Delete`, `.enable delete`]], 
            msg, 
            {
...
global.dfail = async (type, m, conn) => {
    ...
    let lang = db.data.users[m.sender].language 
    let msg = {
        rowner: `This Order Is For @${global.owner[0]}`,
        owner: `This Order Is For @${global.owner[0]}`,
        mods: `This Order Is For Moderator`,
        moderator: `This Order Is For Moderator`,
        prems: `This Order Is For Pengguna Premium`,
        premium: `This Order Is For Pengguna Premium`,
        ...
        admin: `This Order Is For Admin Grup`,
        botAdmin: `This Command Is Active When The Bot Becomes Admin`,
        unreg: `Not Registered, Please Register By Typing #register name.age`,
        dewasa: `ADULT feature is not active, please contact the owner to activate it`,
        ...
        rpg: `RPG features are not active, please contact the owner to activate it`,
...
global.tmbnld = [
    'https://telegra.ph/file/1245227d550a493c2c516.jpg',
]
global.tmbnlm = [
    'https://telegra.ph/file/1245227d550a493c2c516.jpg',
    'https://telegra.ph/file/1245227d550a493c2c516.jpg',
