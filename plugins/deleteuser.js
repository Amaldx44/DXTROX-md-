let handler = async (m, { conn, usedPrefix, command, text }) => {
  ...
  if(!text && !m.quoted) return conn.reply(m.chat, `what is the number?\example: *${usedPrefix}${command} ${global.owner[0]}*\n@tag/reply user`, m)
  ...
  try {
	...
	delete global.db.data.users[user]
        ...
        let anu = `Delete successfully@${number} from *DATABASE*`
